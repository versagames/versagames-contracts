...
contract SmartCraftInitializable is Ownable, ReentrancyGuard {
    ...
    uint256 public accToken2PerShare;
    ...
    struct UserInfo {
        ...
        uint256 reward2Debt; // Reward2 debt
    }
    ...
    event EmergencyReward2Withdraw(address indexed user, uint256 amount);
    event NewStartAndEndBlocks(uint256 startBlock, uint256 endBlock);
    event NewRewardPerBlock(uint256 rewardPerBlock);
    event NewReward2PerBlock(uint256 reward2PerBlock);
    event NewDualRewardPerBlock(uint256 rewardPerBlock, uint256 reward2PerBlock);
     ...
     * @param _reward2Token: reward2 token address
     * @param _rewardPerBlock: reward per block (in rewardToken)
     * @param _reward2PerBlock: reward per block (in reward2Token)
    ...
    function initialize(
        ...
        uint256 decimalsReward2Token = uint256(reward2Token.decimals());
        require(decimalsReward2Token < 36, "Must be inferior to 36");

        PRECISION_FACTOR2 = 10**(uint256(36).sub(decimalsReward2Token));
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            uint256 pending2 = user.amount.mul(accToken2PerShare).div(PRECISION_FACTOR2).sub(user.reward2Debt);
            if (pending2 > 0) {
                reward2Token.safeTransfer(msg.sender, pending2);
            }
        }
    ...
    /*
     * @notice Withdraw staked tokens and collect staked tokens
     * @param _amount: amount to withdraw (in stakedToken)
     */
    ...
    function emergencyReward2Withdraw(uint256 _amount) external onlyOwner {
        reward2Token.safeTransfer(msg.sender, _amount);
        emit EmergencyReward2Withdraw(msg.sender, _amount);
    }
    ...
    function recoverWrongTokens(ERC20 _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        ...
        require(_tokenAddress != reward2Token, "Cannot be reward2 token");
    ...
    function updateRewardPerBlockAfterStart(uint256 _rewardPerBlock, uint256 _reward2PerBlock) external onlyOwner {
        ...
        rewardPerBlock = _rewardPerBlock;
        reward2PerBlock = _reward2PerBlock;
        emit NewDualRewardPerBlock(_rewardPerBlock, _reward2PerBlock);
    }
    /**
    ...
    function pendingReward2(address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        uint256 stakedTokenSupply = stakedToken.balanceOf(address(this));
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
            uint256 stakedTokenReward2 = multiplier.mul(reward2PerBlock);
            uint256 adjustedToken2PerShare =
                accToken2PerShare.add(stakedTokenReward2.mul(PRECISION_FACTOR2).div(stakedTokenSupply));
            return user.amount.mul(adjustedToken2PerShare).div(PRECISION_FACTOR2).sub(user.reward2Debt);
        } else {
            return user.amount.mul(accToken2PerShare).div(PRECISION_FACTOR2).sub(user.reward2Debt);
        }
    }
    ...
    function _updatePool() internal {
        ...
        uint256 stakedTokenReward2 = multiplier.mul(reward2PerBlock);
        accToken2PerShare = accToken2PerShare.add(stakedTokenReward2.mul(PRECISION_FACTOR2).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
